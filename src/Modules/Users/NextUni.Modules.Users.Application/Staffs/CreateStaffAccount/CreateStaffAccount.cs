...
public class CreateStaffAccount
{
        ...
        : ICommandHandler<Command, Guid>
    {
        public async Task<Result<Guid>> Handle(Command command, CancellationToken cancellationToken)
        {
            ...
            var staffRole = dbContext.Roles.Attach(Role.Staff);
            user.Roles.Add(staffRole.Entity);
            user.Raise(new StaffAccountCreatedDomainEvent(user.Id, command.UniversityId));
